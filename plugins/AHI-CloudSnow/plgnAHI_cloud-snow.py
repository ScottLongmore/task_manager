#!/usr/bin/python
"""
  plgnAHI_cloud-snow.py - plug-in methods: TASKS, WORK, PURGE
"""
...
def WORK(config, task):
    ...
    for sectorName in plugin['sectors']:

       sector=plugin['sectors'][sectorName] 
       LOG.info('Converting {} sector to TDF file'.format(sectorName))

       # Create sector sub work directory
       sectorWorkDir=os.path.join(workDir,sectorName)
       try:
       ...
       commandArgs=[dataDir,                # $satfile_path = $ARGV[0];
                    taskDTG.strftime("%Y"), # $year         = $ARGV[1];
                    taskDTG.strftime("%m"), # $month        = $ARGV[2];
                    taskDTG.strftime("%d"), # $day          = $ARGV[3];
                    taskDTG.strftime("%H"), # $hour         = $ARGV[4];
                    taskDTG.strftime("%M"), # $min          = $ARGV[5];
                    sectorName,             # $sector       = $ARGV[6];
                    sector['lines'],        # $lines        = $ARGV[7];
                    sector['line_start'],   # $line_start   = $ARGV[8];
                    sector['line_end'],     # $line_end     = $ARGV[9];
                    sector['samples'],      # $samples      = $ARGV[10];
                    sector['samp_start'],   # $samp_start   = $ARGV[11];
                    sector['samp_end'],     # $samp_end     = $ARGV[12];
                    plugin['binDir'],       # $PROG_DIR     = $ARGV[13];
                    sectorWorkDir,          # $WORK_DIR     = $ARGV[14];
                    tdfDir                  # $output_path  = $ARGV[15];
                   ]
       ...
       # Run AHI Cloud Snow processing script 
       commandList=[plugin['AHICloudSnowScp']]
       commandArgs=[tdfFilePath,                   # $file_qualified = $ARGV[0];
                    sectorName,                    # $SECTOR_NAME    = $ARGV[1];
                    plugin['AHICloudSnowScpDir'],  # $SCRIPT_PATH    = $ARGV[2];
                    plugin['imgDir'],              # $ARCHIVE_PATH   = $ARGV[3];
                    sectorWorkDir                  # $SCRATCH_PATH   = $ARGV[4];
                   ]

       commandID=taskDTG.strftime(DTSFormat)
       if not execute(commandList,commandArgs,commandID,workDir):
           LOG.warning("Execute failed for {}".format(plugin['AHICloudSnowScp']))
           status=False
           return(status)
